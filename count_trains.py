"""
Counts total trains across all filtered daily JSON files, and checks
how many ended up >5 minutes late at their final stop vs. on time.

Usage:
    python count_trains.py filtered/
"""
...
def get_final_delay(train: dict) -> int | None:
    """Return differenceInMinutes of the train's last timeTableRow, or None if missing."""
    rows = train.get("timeTableRows", [])
    if not rows:
        return None
    last_row = rows[-1]
    return last_row.get("differenceInMinutes")
